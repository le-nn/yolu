using Hadron.Buffers;
using Hadron.Collections;
using Hadron.Numerics;
...
public partial class RandomUtils {
    ...
    private static bool CleanupInternalBuffer => !LibrarySettings.DisableRandomStringInternalBufferCleanup;
    ...
    private ref struct CachedRandomNumberGenerator<TRandom>
        ...
        private readonly TRandom randomBytesSource;
        ...
        private ulong Reject(ulong m, uint maxValue) {
            var t = unchecked(0U - maxValue) % maxValue;
    ...
    private static void Next<TRandom, T>(TRandom random, ReadOnlySpan<T> allowedInput, Span<T> buffer)
        ...
        using var randomVectorBuffer = (uint)buffer.Length <= (uint)SpanOwner<uint>.StackallocThreshold
            ? stackalloc uint[buffer.Length]
            : new SpanOwner<uint>(buffer.Length);
